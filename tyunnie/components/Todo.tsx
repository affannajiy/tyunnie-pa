...
const TAGS = [
  { value: 'cs',       label: 'CS / Code', className: 'bg-blue-50 text-blue-500 border-blue-200'       },
  { value: 'write',    label: 'Writing',   className: 'bg-[#fff0e6] text-[#c2500f] border-[#fed7aa]'   },
  { value: 'personal', label: 'Personal',  className: 'bg-purple-50 text-purple-500 border-purple-200' },
  { value: 'other',    label: 'Other',     className: 'bg-[#f3f0ea] text-[#9a8f7e] border-[#e8e2d8]'  },
]
...
export default function Todo({ userId, onAction, refreshKey }: Props) {
  ...
  async function handleToggle(id: string, currentDone: boolean) {
    ...
    if (newDone) onAction("Yes!! One down — you're unstoppable 🔥")
  }
    ...
    <div>
      ...
      <div className="bg-white border border-[#e8e2d8] rounded-2xl p-5 mb-5">
        <div className="flex items-center gap-3 mb-4">
          <span className="font-serif italic text-[#f97316] text-sm">New Task</span>
        ...
        <div className="flex flex-col gap-2">
          {filtered.map(todo => (
            ...
            >
              ...
              {/* Text */}
              <span className={`flex-1 text-sm font-medium ${todo.done ? 'line-through text-[#9a8f7e]' : 'text-[#111010]'}`}>
                {todo.text}
              </span>
              ...
              {todo.due && (
                ...
                `}>
                  {isOverdue(todo.due) && !todo.done ? '⚠ ' : ''}{todo.due}
                </span>
              )}
