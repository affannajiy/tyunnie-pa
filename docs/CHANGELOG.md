...
- **Tyunnie called you by your old name after renaming yourself** (`components/Desk.tsx`, `components/TyunniePanel.tsx`) — the desk one-liner and the daily briefing are AI-generated *text* with the name baked in, cached in `sessionStorage` under a fixed key that nothing invalidated. The greeting above updated live while the line below stayed frozen from before the rename. Both caches are now keyed on the active name (`desk_oneliner:<name>:<date>`, `tyunnie_briefing:<name>`), so a rename simply misses and regenerates — no invalidation call to forget on some future code path. The date component also fixes a second staleness class: a session left open past midnight showed yesterday's line against yesterday's task counts. Both effects now depend on the name, which additionally closes a race where a profile row resolving *after* mount cached a nameless line for the whole session. No name was ever hardcoded.

- **Usability & accessibility sweep** — first pass against the new [usability contract](USABILITY_HEURISTICS.md):
  - **Global `:focus-visible` ring** (`app/globals.css`) in the user's accent colour. 63 `outline-none` call sites previously left keyboard users with *no* focus indicator anywhere in the app; one global rule fixes all of them without adding a ring for mouse users.
...
- **Todo completion confetti ignored your accent** (`components/Todo.tsx`) — the palette was a hardcoded JS array `["#f97316", "#fed7aa", "#c2500f", "#fff0e6"]` handed to canvas-confetti. The "ACCENT COLOR OVERRIDES" block in `app/globals.css` remaps hardcoded Tailwind *classes* onto the accent vars, but it only ever reaches classes — never a JS array — so confetti stayed orange under any custom accent. It now derives its four colours from the live accent via the new `readAccentVar()` helper.
...
- **Daily note trimmed to one line** (`app/api/daily-quote/route.ts`) — the prompt asked for "1-3 sentences" and the model always took 3, so the ceiling had become the target. Now: normally one sentence, two only if it earns it, never three; `max_tokens` 140 → 70. The `— Taehyun` sign-off is dropped (his name was already in the From line *and* the subject — three times per email), with a `stripSignOff()` guard for the days the model adds one anyway. Email chrome lightened: the heavy orange bordered card is gone so a single line reads as a note, not a certificate. Topic/tone variety pools are untouched.
...
- **Tetris board respects dark mode** (`components/games/Tetris.tsx`) — its background was an inline `backgroundColor`, and dark mode here works by overriding light *utility classes*, which an inline style bypasses entirely. Moved to `bg-[#faf8f5]` so the `.dark` override reaches it.
